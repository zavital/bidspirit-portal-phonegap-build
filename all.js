        ...
        function onResize() {
            if (!DomUtilsService.isTextInputFocused()) {
                ...
                mResizeTriggerTimer = $timeout(function() {
                    ...
                    adjustViewPortAccordingToOrientation(), updateViewportInfo());
                }, 300);
            }
        }
...
}), define("common/js/modules/domUtils/index", [ "./domUtilsModule", "./domUtilsService", "./enterKeyActionDirective", "./viewPortService", "./slideToggleDirective", "./autoFontSizeDirective", "./currentDirectionDirective", "./scrollOnDirective", "./backButtonDirective", "./bsWidthLimitDirective", "./focusOnDirective", "./linkableTextDirective" ], function() {}), 
...
define("common/js/modules/i18n/i18nModule", [ "angular" ], function(ng) {
    ...
    module.factory("I18nService", function($interpolate, $http, $log, $q, $filter, $rootScope, StringsService, LocalStorageService, PathsService, SettingsService, DateUtilsService) {
        ...
        var DEFAULT_LANG = "en", mCurrentLang = null, mTextsByLang = {}, mResourcePathFn = null, mSupportedLangs = [ "he", "en", "ru" ];
        return {
            init: init,
...
define("common/js/modules/catalogUtils/catalogUtilsModule", [ "angular" ], function(ng) {
    ...
    module.factory("CatalogUtilsService", function(StringsService, ArraysService, I18nService, DateUtilsService) {
        ...
        function getLotPage(items, lotId, lotsPerPage) {
            var lotInd = ArraysService.getIndById(items, lotId);
            return null == lotInd ? 1 : Math.ceil((lotInd + 1) / lotsPerPage);
        }
        function getPageItems(allItems, page, lotsPerPage) {
            if (!allItems) return [];
            var startInd = (page - 1) * lotsPerPage, endInd = startInd + 1 * lotsPerPage;
...
define("portal/js/modules/main/portalMainModule", [ "angular" ], function(ng) {
    ...
    module.factory("PortalInfoService", function($q, $http, $rootScope, $interval, ApiService, ArraysService, I18nService, SettingsService, StringsService, LocalStorageService, LogService, DateUtilsService, SessionsService, CachedApiService) {
        ...
        function getBidSpiritHouses() {
            return sortHouses(), ArraysService.getFilteredList(mHouses, "bidspiritSiteId", !1, null);
        }
        ...
        function checkNearAuctionStates() {
            if (mAuctions) {
                for (var auctionsToCheck = [], i = 0; i < mAuctions.length; i++) {
                    var auction = mAuctions[i], minutesUntilAuction = getMinutesUntilAuction(auction);
                    60 > minutesUntilAuction && "READY" == auction.state && minutesUntilAuction > -1440 && auctionsToCheck.push(auction.id);
                }
                auctionsToCheck.length > 0 && ApiService.callApi("/portal/getAuctionsStates", {
                    auctionsIds: auctionsToCheck
                }).success(function(auctionsStates) {
                    var changed = !1;
                    for (var auctionId in auctionsStates) {
                        var auction = getAuction(auctionId);
                        auction.state != auctionsStates[auctionId] && (auction.state = auctionsStates[auctionId], 
                        changed = !0);
                    }
                    changed && $rootScope.$broadcast("auctions.newStates", auctionsStates);
                });
            }
        }
        return mHouses = [], mHousesMap = {}, mAuctions = [], mAuctionsMap = {}, mHousesAuctions = {}, 
        mInfo = {}, $interval(checkNearAuctionStates, 6e4), $rootScope.$on("i18n.languageChanged", sortHouses), 
        {
            init: init,
...
}), define("portal/js/modules/main/index", [ "./portalMainModule", "./portalMainController", "./portalInfoService", "./portalTextsService", "./portalStates" ], function() {}), 
define("portal/js/modules/auth/portalAuthModule", [ "angular" ], function(ng) {
    ...
    module.factory("PortalAuthService", function($q, $rootScope, $state, $interval, $log, $modal, $modalStack, AnalyticsService, LocalStorageService, PathsService, ApiService, PopupsService, SessionsService, PortalInfoService) {
        ...
        function checkUserSession() {
            ...
            if (user) {
                var options = {};
                "COMPLETE" != user.registrationStage && (options.withRegistrationStage = !0), ApiService.callApi("/auth/checkSession", options).success(function(sessionCheckInfo) {
                    sessionCheckInfo.sessionEnded ? ($log.info("session ended."), SessionsService.setSessionUser(null)) : options.withRegistrationStage && user.registrationStage != sessionCheckInfo.registrationStage && (user.registrationStage = sessionCheckInfo.registrationStage, 
        ...
        return $interval(checkUserSession, 6e4), $rootScope.$on("auth.upperNavPopupDisplay", function(event, navAuthPopupOn) {
...
define("portal/js/modules/auctions/auctionsModule", [ "angular" ], function(ng) {
    ...
    module.controller("HomeController", [ "$scope", "$rootScope", "$interval", "$window", "ArraysService", "I18nService", "ViewPortService", "SessionsService", "PortalInfoService", "SearchService", function($scope, $rootScope, $interval, $window, ArraysService, I18nService, ViewPortService, SessionsService, PortalInfoService, SearchService) {
        ...
        $window.addEventListener("resize", adjustUpperPartHeight);
    } ]);
...
define("portal/js/modules/auctions/catalogs/catalogsModule", [ "angular" ], function(ng) {
    ...
    module.factory("CatalogsService", function($q, $filter, ApiService, I18nService, StringsService, ArraysService, DateUtilsService, LocalStorageService, CloudinaryService, CatalogUtilsService) {
        ...
        function resetNavState(auction, items) {
            ...
            var storedLotsPerPage = LocalStorageService.load("lotsPerPage");
            storedLotsPerPage && (mNavState.pagesData.lotsPerPage = 1 * storedLotsPerPage);
        }
        function updatePageItems() {
            var pagesData = mNavState.pagesData;
            pagesData.visibleItems && (pagesData.pageItems = CatalogUtilsService.getPageItems(pagesData.visibleItems, pagesData.currentPage, pagesData.lotsPerPage), 
            LocalStorageService.store("lotsPerPage", pagesData.lotsPerPage));
        }
...
define("portal/js/modules/auctions/catalogs/list/catalogListModule", [ "angular" ], function(ng) {
    ...
    module.controller("CatalogListController", [ "$rootScope", "$scope", "$timeout", "$state", "$stateParams", "AnalyticsService", "ArraysService", "LocalStorageService", "I18nService", "DomUtilsService", "CatalogUtilsService", "StructuredDataService", "PortalInfoService", "CatalogsService", "CatalogAccountService", function($rootScope, $scope, $timeout, $state, $stateParams, AnalyticsService, ArraysService, LocalStorageService, I18nService, DomUtilsService, CatalogUtilsService, StructuredDataService, PortalInfoService, CatalogsService, CatalogAccountService) {
        ...
        function setScroll() {
            if (0 == previousState.url.indexOf("lotPage")) {
                ...
                $scope.pagesData.currentPage = CatalogUtilsService.getLotPage($scope.pagesData.visibleItems, lotId, $scope.pagesData.lotsPerPage), 
        ...
        function watchNavParams() {
            $scope.$watch("pagesData.lotsPerPage", onNavParamChange), $scope.$watch("pagesData.currentPage", onNavParamChange), 
    ...
    module.directive("bsCatalogListPagination", function($timeout, $rootScope, $stateParams, PathsService) {
        return {
            ...
            link: function(scope, element) {
                function calcMaxButton(availableWidth) {
                    return Math.floor(availableWidth / 50) - 4;
                }
                function updateDisplay() {
                    scope.currentLang = $rootScope.currentLang, adjustButtonsNumber();
                }
                function adjustButtonsNumber() {
                    var parent = element.parent()[0];
                    if (parent) {
                        var parentWidth = parent.offsetWidth, availableWith = Math.min(850, parentWidth);
                        parentWidth > 800 && "upper" == scope.positionInPage && $rootScope.viewPort.pcMedia ? (availableWith -= 350, 
                        scope.lifted = !0) : (availableWith -= 50, scope.lifted = !1, element.removeClass("lifted"));
                        var maxButtons = calcMaxButton(availableWith);
                ...
                scope.hrefPages && $timeout(setHrefs, 200), updateDisplay(), element.addClass(scope.positionInPage), 
...
define("portal/js/modules/auctions/catalogs/search/searchModule", [ "angular" ], function(ng) {
    ...
    module.factory("SearchService", function($q, $rootScope, $state, ApiService, I18nService, StringsService, ArraysService, PortalInfoService) {
        function searchAllCatalogs(token, time, limit, page) {
            ...
            return ApiService.callApi("/portal/searchItems/", {
                ...
                allAuctionsFound ? deferred.resolve(response) : PortalInfoService.loadForRegion($rootScope.currentRegion).then(function() {
        ...
        var mLastSearchInfo = {}, mNewSearchInfo = {};
        return {
    ...
    module.controller("SearchController", [ "$scope", "$timeout", "$state", "CatalogUtilsService", "AnalyticsService", "ArraysService", "I18nService", "LogService", "StringsService", "AppSiteWinodwsService", "AccountService", "SearchService", "PortalInfoService", function($scope, $timeout, $state, CatalogUtilsService, AnalyticsService, ArraysService, I18nService, LogService, StringsService, AppSiteWinodwsService, AccountService, SearchService) {
        function getSearchResultsForCurrentPage() {
            ...
            SearchService.searchAllCatalogs($scope.data.token, $scope.data.searchTime, $scope.pagesData.lotsPerPage, $scope.pagesData.currentPage);
        }
        ...
        function setItems(items) {
            ...
            }), $scope.pagesData.pageItems = ArraysService.filteredByNonEmpty(items, "auction").slice(0, $scope.pagesData.lotsPerPage), 
...
define("portal/js/modules/auctions/lists/auctionsListsModule", [ "angular" ], function(ng) {
        ...
        return {
            ...
            link: function(scope) {
                ...
                function initDisplayInfo() {
                    ...
                    scope.imageSize = getImageSize(), angular.forEach(scope.auctions, function(auction) {
                        var catalogOpenText = "home_auction_catalog_open", buttonClass = "orange";
                        switch (auction.state) {
                          case "PENDING":
                            catalogOpenText = "home_auction_catalog_not_ready", buttonClass = "disabled";
                            break;

                          case "READY":
                            auction.externalUrl || auction.catalogInfo ? (catalogOpenText = "home_auction_catalog_open", 
                            buttonClass = "orange", auction.catalogInfo && !auction.timedAuction && (auction.buttonAsLink = !0)) : (catalogOpenText = "home_auction_catalog_not_ready", 
                            buttonClass = "disabled");
                            break;

                          case "RUNNING":
                            catalogOpenText = "home_auction_enter", buttonClass = "live darkBlue";
                            break;

                          case "ENDED":
                            auction.catalogInfo ? (catalogOpenText = "home_auction_catalog_result", buttonClass = "yellow", 
                            auction.buttonAsLink = !0) : auction.removed = !0;
                        }
                        auction.catalogOpenText = catalogOpenText, auction.buttonClass = buttonClass;
                    });
                }
                scope.scrollTo = getAuctionToScrollTo(), scope.devMode = $rootScope.devMode, scope.viewPort = $rootScope.viewPort, 
                scope.w = ViewPortService.getViewPortWidth(), scope.isAuctionClickable = function(auction) {
                ...
                }, initTimeInfo(), initDisplayInfo(), $rootScope.$on("auctions.newStates", initDisplayInfo);
            },
    ...
    module.directive("bsAuctionsListsGroup", function($timeout, $rootScope, $state, PathsService, PortalInfoService, SessionsService, ArraysService, DateUtilsService, SettingsService) {
        return {
            ...
            link: function(scope) {
                ...
                var daysToFutureAuctions = SettingsService.get("daysToFutureAuctions") || 14, recentAuctionsToShow = SettingsService.get("recentAuctionsToShow") || 10, auctions = scope.auctions;
                if (auctions) {
                    for (var i = 0; i < auctions.length; i++) {
    ...
    module.controller("AuctionsResultsController", [ "$scope", "$rootScope", "$timeout", "ArraysService", "I18nService", "SessionsService", "PortalInfoService", function($scope, $rootScope, $timeout, ArraysService, I18nService, SessionsService, PortalInfoService) {
        function setAuctions() {
            ...
            }), ArraysService.inverse($scope.pastAuctions), $scope.displayedAuctions = $scope.pastAuctions;
        }
        function setHouses() {
        ...
        $scope.filterByHouse = function(house) {
            $scope.displayedAuctions = getHouseAuctions(house), $scope.currentHouse = house, 
            $scope.updateLogo = !1, window.scroll(0, 0), $timeout(function() {
                $scope.updateLogo = !0;
            }, 200);
...
define("portal/js/modules/account/accountModule", [ "angular" ], function(ng) {
    ...
    module.controller("FavoritesController", [ "$rootScope", "$scope", "$filter", "$state", "$timeout", "ArraysService", "I18nService", "StringsService", "PortalInfoService", "AccountService", "CatalogUtilsService", "AppSiteWinodwsService", function($rootScope, $scope, $filter, $state, $timeout, ArraysService, I18nService, StringsService, PortalInfoService, AccountService, CatalogUtilsService, AppSiteWinodwsService) {
        function setPageItems() {
            $scope.pagesData.pageItems = CatalogUtilsService.getPageItems($scope.data.sceneInfo.items, $scope.pagesData.currentPage, $scope.pagesData.lotsPerPage);
        }
        function scrollToLastClickedLot() {
            ...
            lotToScrollTo && ($scope.pagesData.currentPage = CatalogUtilsService.getLotPage($scope.data.sceneInfo.items, lotToScrollTo.id, $scope.pagesData.lotsPerPage), 
        ...
        $scope.data = {}, $scope.onPageChange = function() {
            ...
            AccountService.getFavoriteItems("PAST").then(function(pastItems) {
                if (pastItems.length > 0) {
                    ...
                    $scope.pagesData.currentPage = CatalogUtilsService.getLotPage($scope.data.sceneInfo.items, pastItems[0].id, $scope.pagesData.lotsPerPage), 
